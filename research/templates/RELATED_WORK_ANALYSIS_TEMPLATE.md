...
이 템플릿은 주로 **Track A — Core Method / Detection** 논문에 사용한다. 개념·이론·경제 모델·survey·실증·공식 사양이 주목적인 논문은 먼저 역할을 분류하고 `CONCEPT_REFERENCE_ANALYSIS_TEMPLATE.md`를 사용하거나 필요한 항목만 선택한다.

논문마다 이 파일을 복사해 해당 논문 폴더의 `관련 연구 분석.md` 또는 저장소가 정한 paper-note 위치에 작성한다.
...
- [ ] `research/RESEARCH_GAP_LEDGER.md`에서 기존 gap의 지원·약화·반박 결과 갱신
- [ ] 재사용 가능한 정의·수식·실증 결과·반례를 `research/CITATION_IDEA_BANK.md`에 등록
- [ ] 직접 비교 대상이면 `PMA-논문/related-work-comparison.md`를 Evidence ID와 함께 갱신
- [ ] 현재 `ACCEPTED` 결정과 충돌 여부 확인
- [ ] 충돌 시 결정을 자동 변경하지 않고 반례·영향·재검토 후보 제시
- [ ] 추가로 읽어야 할 반증 논문과 artifact 기록
